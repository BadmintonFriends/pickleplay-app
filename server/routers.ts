...
const tournamentRouter = router({
  ...
  registrationStatus: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      return events.map(e => {
        const count =
          counts.find(c => c.tournamentEventId === e.id)?.count ?? 0;
        return {
          eventId: e.id,
...
const registrationRouter = router({
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const counts = await db.getEventRegistrationCounts(input.tournamentId);
      const currentCount =
        counts.find(c => c.tournamentEventId === event.id)?.count ?? 0;
      if (currentCount >= event.maxTeams)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "해당 종목의 접수가 마감되었습니다",
        });

      const isSingles =
        event.eventType === "남단" || event.eventType === "여단";
      if (isSingles && input.players.length !== 1)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "단식 종목은 1명만 등록 가능합니다",
        });
      if (!isSingles && input.players.length !== 2)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "복식 종목은 2명을 등록해야 합니다",
        });
      ...
      if (tournament.sizeOptions) {
        ...
        for (const player of input.players) {
          if (player.giftSize && !validSizes.includes(player.giftSize)) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: `유효하지 않은 사이즈입니다: ${player.giftSize}`,
            });
          }
        }
      }
      ...
      const registrationNumber = await db.generateRegistrationNumber(
        input.tournamentId
      );
  ...
  myRegistrations: protectedProcedure.query(async ({ ctx }) => {
    const regs = await db.getRegistrationsByUserOrPlayer(
      ctx.user.id,
      ctx.user.phone ?? null
    );
    const result = [];
  ...
  detail: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!reg)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "접수 내역을 찾을 수 없습니다",
        });
      if (
        reg.userId !== ctx.user.id &&
        !["admin", "super_admin"].includes(ctx.user.role)
      ) {
        // 대회 관리자인지 확인
        const isOrganizer = await db.isTournamentOrganizer(
          reg.tournamentId,
          ctx.user.id
        );
        if (!isOrganizer) {
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        ...
        if (!isOrganizer) {
          ...
          const isPlayerInReg =
            userPhone &&
            playerList.some(p => p.phone.replace(/\D/g, "") === userPhone);
          if (!isPlayerInReg) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "권한이 없습니다",
...
const adminRouter = router({
  ...
  updatePaymentStatus: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.paymentStatus === "paid") {
        try {
          const playerList = await db.getPlayersByRegistration(
          ...
          const mainPlayer =
            playerList.find(p => p.playerOrder === 1) || playerList[0];
          if (mainPlayer?.phone) {
  ...
  updateRegistrationEvent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.ageGroupId !== undefined)
        updateData.ageGroupId = input.ageGroupId;
  ...
  reorderEvents: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await verifyTournamentOwnership(ctx.user, input.tournamentId);
      const orders = input.eventIds.map((id, index) => ({
        id,
        sortOrder: index,
      }));
  ...
  addTournamentOrganizer: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // 이미 등록된지 확인
      const isAlready = await db.isTournamentOrganizer(
        input.tournamentId,
        input.userId
      );
      if (isAlready) {
      ...
      await db.addTournamentOrganizer(
        input.tournamentId,
        input.userId,
        "manager"
      );
  ...
  getUserManagedTournaments: protectedProcedure.query(async ({ ctx }) => {
  ...
  login: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!user) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "가입되지 않은 번호입니다. 회원가입을 진행해주세요.",
        });
      }
