...
export default function MyPage() {
  ...
  const registrationsQuery = trpc.registration.myRegistrations.useQuery(
    undefined,
    { enabled: isAuthenticated }
  );
  ...
  const deleteAccountMutation = trpc.user.deleteAccount.useMutation({
    onSuccess: () => {
      ...
      void utils.registration.myRegistrations.invalidate();
      navigate("/");
    },
    onError: err => toast.error(err.message || "탈퇴 처리에 실패했습니다"),
  });
  ...
  return (
    <div className="bg-background min-h-screen pb-24">
      ...
      <div className="px-4 pt-1 pb-3 flex items-center gap-2">
        <button
          onClick={() => navigate("/")}
          className="w-8 h-8 rounded-full bg-card border border-line-strong flex items-center justify-center"
        >
      ...
      <div className="px-4 space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={editBirthDate}
                    onChange={e =>
                      setEditBirthDate(formatBirthDate(e.target.value))
                    }
                    maxLength={10}
                    className="bg-ink-3 border-line-strong text-foreground"
                  />
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="flex-1 bg-primary hover:bg-optic-deep text-primary-foreground"
                  >
          ...
          {registrationsQuery.isLoading ? (
            ...
            <div className="space-y-3">
              {registrations.map((reg: any) => {
                ...
                const payment =
                  paymentLabels[reg.paymentStatus] || paymentLabels.unpaid;
                return (
                  <Card
                    key={reg.id}
                    className="border-line-strong bg-card rounded-2xl cursor-pointer hover:border-primary/40 transition-colors"
                  ...
                  >
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        <Badge
                          className={`${status.color} text-xs px-2 py-0.5 rounded-full font-medium`}
                        >
                          {status.label}
                        </Badge>
                        <Badge
                          className={`${payment.color} text-xs px-2 py-0.5 rounded-full font-medium`}
                        >
                          {payment.label}
                        </Badge>
                        {reg.eventType && (
        ...
        <div className="space-y-2 pt-2">
          ...
          >
            ...
            <span className="text-sm text-secondary-foreground flex-1 text-left">
              이용약관
            </span>
          ...
          >
            ...
            <span className="text-sm text-destructive flex-1 text-left">
              로그아웃
            </span>
          </button>
        </div>
          ...
          <div className="pt-4 text-center">
            <AlertDialogTrigger asChild>
              <button
                ...
                className="text-xs text-muted-foreground underline underline-offset-4 decoration-muted-foreground/70 hover:text-destructive hover:decoration-destructive disabled:pointer-events-none disabled:opacity-60"
              >
                {deleteAccountMutation.isPending ? "탈퇴 처리 중" : "회원탈퇴"}
              </button>
            </AlertDialogTrigger>
          </div>

          <AlertDialogContent className="rounded-2xl border-line-strong bg-card text-foreground">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-base font-bold">
                회원탈퇴
              </AlertDialogTitle>
              <AlertDialogDescription className="text-sm leading-6 text-muted-foreground whitespace-pre-line">
                {
                  "정말 탈퇴하시겠습니까?\n재가입시 기존 내역은 복구되지 않습니다."
                }
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel className="border-line-strong bg-ink-3 text-secondary-foreground hover:bg-ink">
                취소
              </AlertDialogCancel>
              <AlertDialogAction
